...
export async function getDownloadLink(cleanPath: string, accessToken: string, urlEncode: boolean) {
  ...
  const {
    ...
    size,
    id,
  } = await fetchWithAuth(requestUrl, { method: 'GET', accessToken }).then(res => res.json())
