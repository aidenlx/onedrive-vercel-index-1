...
import { useBatchDownload } from '@ui/utils/useBatchDownload'
...
export function FolderAction({
  ...
  async function handleFolderDownload() {
    ...
    const query = new URLSearchParams({ path: getItemPath(c.name) })
    const url = new URL(`/api/traverse?${query}`, window.location.origin)
